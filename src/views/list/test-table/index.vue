<template>
  <div id="parentNode" class="container">
    ...
    <a-card class="general-card" :title="'查询表格'">
      ...
      <a-table
        ...
        :bordered="false"
        :size="size"
        :row-selection="rowSelection.show ? rowSelection : undefined"
...
<script lang="ts" setup>
  ...
  type Column = TableColumnData & { dataIndex: string; checked?: true };
  ...
  const configChange = (_tableConfig: any) => {
    Object.assign(rowSelection, _tableConfig.rowSelection);
    columns.value = _tableConfig.columns.filter((v: any) => v.tableShow);
  };
  const jump = () => {
    router.push({
      name: 'TestTable',
      params: { pageId: '1234567' },
    });
  };
